...
def train_speedy_model(custom_model, language, train_csv, eval_csv, num_epochs, batch_size, grad_acumm, output_path, max_audio_length):
    ...
    if custom_model and custom_model.startswith("http"):
        custom_model_path = download_file(custom_model, "custom_model_speedy.pth")
        if not custom_model_path:
            return "Failed to download custom SpeedySpeech model!", "", "", "", "", ""
        custom_model = custom_model_path

    # Load custom checkpoint if provided
    if custom_model and os.path.isfile(custom_model):
        print(f"Loading custom SpeedySpeech model from: {custom_model}")
    ...
    try:
        ...
        return f"SpeedySpeech training was interrupted due to an error:\n{error}", "", "", "", "", ""

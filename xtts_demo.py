...
from utils.vits_train import train_vits_model  # NEW IMPORT
...
def preprocess_dataset(audio_path, audio_folder_path, language, whisper_model, out_path, train_csv, eval_csv, progress=gr.Progress(track_tqdm=True)):
    ...
    if not audio_files:
        ...
        try:
            ...
            return f"The data processing was interrupted due an error !! \n Error summary: {error}", "", ""
    if audio_total_size < 120:
        message = "The sum of the duration of the audios should be at least 2 minutes!"
        print(message)
        return message, "", ""
...
def optimize_model(out_path, clear_train_data):
    ...
    if not model_path.is_file():
        # If optimizing for VITS - it might have saved as model.pth directly
    ...
    # remove dvae for xtts if present
...
def handle_training(custom_model, model_selection, version, language, train_csv, eval_csv, num_epochs, batch_size, grad_acumm, output_path, max_audio_length):
    clear_gpu_cache()
    if model_selection == "XTTS":
        # Train XTTS
        return train_gpt(custom_model, version, language, train_csv, eval_csv, num_epochs, batch_size, grad_acumm, output_path, max_audio_length)
    elif model_selection == "VITS":
        # Train VITS
        return train_vits_model(custom_model, language, train_csv, eval_csv, num_epochs, batch_size, grad_acumm, output_path, max_audio_length)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--whisper_model",
        type=str,
        help="Name of the whisper model selected by default",
        default="large-v3",
    )
    parser.add_argument(
        "--audio_folder_path",
    ...
    with gr.Blocks(title=os.environ.get("APP_NAME", "Gradio")) as demo:
        with gr.Tab("1 - Data processing"):
            ...
            upload_file = gr.File(
                file_count="multiple",
                label="Audio files for XTTS/VITS training (wav, mp3, flac)",
            )
        ...
        with gr.Tab("2 - Fine-tuning TTS Model"):
            model_selection = gr.Dropdown(
                label="Select TTS model to fine-tune",
                value="XTTS",
                choices=["XTTS", "VITS"]
            )

            version = gr.Dropdown(
                label="XTTS base version (ignored if VITS selected)",
                value="v2.0.2",
                choices=[
                    "v2.0.3",
                    "v2.0.2",
            ...
            custom_model = gr.Textbox(
                label="(Optional) Custom model.pth file or URL (leave blank if using base model)",
                value="",
            )
            ...
            clear_train_data = gr.Dropdown(
                label="Clear train data after optimizing",
                value="none",
                choices=[
                    "none",
                    "run",
        ...
        with gr.Tab("3 - Inference"):
            with gr.Row():
                ...
                with gr.Column():
                    ...
                    tts_text = gr.Textbox(
                        label="Input Text.",
                        value="This model sounds really good and it's reasonably fast.",
                    )
                    with gr.Accordion("Advanced settings", open=False):
                        ...
                        use_config = gr.Checkbox(
                            label="Use Inference settings from config",
                            value=False,
                        )
